...
- **核心功能**：AI 引导生成、提示词优化、个人库与版本管理、多认证、双数据库、响应式布局。
...
- **关键工作流**：
  1. 认证：`authStore` 负责 Linux.do / 飞书 OAuth code 登录、本地用户名密码登录、token 刷新、用户恢复。
...
- **核心能力点**：
  ...
  2. **多认证**：`apps/modules/auth/views.py` 提供 Linux.do `/api/auth/linux-do/login`、飞书 `/api/auth/feishu/login`、本地 `/api/auth/local/login`/`register` 入口；`LinuxDoOAuth`、`FeishuOAuth`、`PasswordUtil`、`JWTUtil` 提供支持。
...
- **认证链路**：前端通过 `/api/auth/config` 获取可用认证方式；Linux.do、飞书回调页面负责提取 `code` 并分别调用 `/api/auth/linux-do/login` 与 `/api/auth/feishu/login`；本地模式通过 `/api/auth/local/login` 与 `/register` 完成。
